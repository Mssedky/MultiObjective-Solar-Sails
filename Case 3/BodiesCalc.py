...
def orbital_elements_to_cartesian(a, e, i, omega, Omega, nu):
    ...
    z = r * np.sin(omega + nu) * np.sin(i)  # Adjusted calculation for z-coordinate
    return x, y, z
...
class orbitingObject:
    def __init__(self, a, e, i, omega, Omega, nuMean, mass, name, positions, velocities):
        ...
        name: options: 'Earth', 'NEO', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto'
        positions:
        velocities:
        '''
        self.a = a
...
def calcCelestialTraj(body, dT, T):
    t = 0  # seconds
    dT = float(dT)
    dT_sim = 0.1  # days
